...
Este repositório é o **harness** (`ai-harness`): constituição + skills + [`rules/`](./rules/) + [`hooks/`](./hooks/) + [`mcp/`](./mcp/) + [`subagents/`](./subagents/). Skill é o HOW do recorte. Rule é o gate de processo em todo projeto. Hook é o enforcement no host. Subagent é o papel despachado pelo host (Cursor / Claude). MCP da **máquina** (catálogo universal + wrappers) mora em [`mcp/`](./mcp/). Segredo só em `~/.config/ai-harness/secrets/*.env` (contrato em `mcp/secrets.example/`). HOW, rule, hook, MCP e subagent **de um cliente** vivem num repositório **privado** irmão (`{cliente}-harness`); o `install.sh` dele projeta em `~/.config/ai-harness/overlay/{mcp,hooks,rules,subagents}/` e nas skills do workspace daquele cliente — nunca neste git público e nunca em `~/.grok/skills`. Skill `client-harness`. O que é só desta máquina e de vários clientes (VPS, AWS pessoal) permanece no overlay local. O `install.sh` liga tudo nos hosts: um canal nativo por provedor, sem o Grok reler Cursor/Claude. Atividade de código (worktree a partir da produção, dual delivery): rule e skill `git-activity`. Defeito/falha: hipóteses concorrentes e refutação antes do patch — rule e skill `debug-hypotheses`. Contrato publicado, breaking change ou trava nova: **pergunte antes** (o quê, quem consome, por quê) — rule `ask-before-contract`.

**Kit de skills** (o que ler neste turno): tabela do Gate 2 em [`rules/analyze-before-implement.md`](./rules/analyze-before-implement.md). Este arquivo **não** copia a tabela — um dono. O pedido mente; o recorte (e o diff) escolhe a linha. Mapa capacidade → skill: `architecture` «Onde mora».
