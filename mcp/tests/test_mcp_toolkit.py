...
class RemoteServerSyncTest(unittest.TestCase):
    ...
    def test_browser_oauth_points_xdg_open_at_harness_chrome_wrapper(self):
        self.catalog_path.write_text(json.dumps({
            "servers": {
                "cloudflare-api": {
                    "command": "npx",
        ...
        _, _, env = toolkit.command_config(toolkit.catalog()["cloudflare-api"])
        xdg_share = str(toolkit.ROOT / "xdg/share")
        scripts = str(toolkit.ROOT / "scripts")
        self.assertTrue(env["XDG_DATA_DIRS"].startswith(xdg_share + ":"))
        self.assertTrue(env["PATH"].startswith(scripts + ":"))
        self.assertEqual(env["BROWSER"], "open-oauth-browser")
