...
**Kit de trabalho** — esta tabela é o dono (processo **e** produto). Constituição e `architecture` **apontam**; não copiam. O pedido mente: se o recorte **contém** o sinal, a linha entra mesmo que o humano não a tenha citado. Pontes (`oauth-connectors`, `langgraph-agents`, `crewai-agents`, `make-scenarios`, `channel-evolution`) não competem aqui.
