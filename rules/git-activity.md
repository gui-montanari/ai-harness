...
- **De onde partir e o nome.** Skill `git-activity`. Sem branch citada neste turno: worktree nova a partir de `origin/master` ou `origin/main`. Branch citada neste turno: seguir essa branch. Nunca de `develop` nem de checkout sujo. Não desenvolver em `main`/`master`/`develop`. Pasta `{YYYYMMDD}-{HHmm}-{kind}-{slug}` (`{repo}` no nome se `worktrees/` for compartilhada), salvo o produto especializar.
